module.exports = (app,con,fs,hummus,Busboy,uuid) => {
    ...
    const lessonSplitPdf = (lesson) => {
      ...
      const getMaxGroupId = () =>{
       ...
       const insertEmployee = (employee) => {
          return new Promise((resolve,reject) => {
            var insertQuery = "INSERT INTO USERS (FIRST_NAME, LAST_NAME, EMAIL,IS_ADMIN) VALUES (@employee.FIRST_NAME,@employee.LAST_NAME,@employee.EMAIL,0)";
                new sql.Request().input('employee.FIRST_NAME', sql.text, employee.FIRST_NAME)
                                .input('employee.LAST_NAME', sql.text, employee.LAST_NAME)
                                .input('employee.Email', sql.text, employee.EMAIL)
                                .query(insertQuery,[
                    employee.FIRST_NAME,
                    employee.LAST_NAME,
                    employee.EMAIL
       ...
       const getAssignmentsByLessonAndGroupId = (group_id,lesson_id) =>{
                return new Promise(async (resolve,reject) => {
              new sql.Request().input('group_id', sql.Int, group_id).input('lesson_id', sql.Int, lesson_id)
              .query('SELECT * FROM ASSIGNMENTS WHERE LESSON_ID= @lesson_id AND GROUP_ID = @group_id',
                        function(err, rows){
        ...
        const deleteAssignmentById = (assignmentId) =>{
            return new Promise(async (resolve,reject) => {
               new sql.Request().input('assignmentId', sql.Int, assignmentId)
               .query("DELETE FROM ASSIGNMENTS where ID = @assignmentId",function(err2,result){
                    if(!err2){
                        resolve({
                            status : true,
        ...
        const getGroupUsers = (group_id) => {
            return new Promise((resolve,reject) => {
              new sql.Request().input('group_id', sql.Int, group_id)
                .query('SELECT USER_ID as user_id FROM GROUPS WHERE ID = @group_id',[group_id], function(err, users){
    ...
    const deleteStatusByGroupAndAssignmentIds = (groupAndAsssignmentIds) =>{
        return new Promise(async (resolve,reject) =>{
            con.query("DELETE FROM STATUS WHERE GROUP_ID = @group_id AND ASSIGNMENT_ID = @assignmentId",groupAndAsssignmentIds,function(err2,result){
                    if(!err2){
                        resolve({
                            status : true,
    ...
    const removeAssignmentLesson = (data) => {
        ...
        return new Promise(async (resolve ,reject) => {
            con.query("SELECT * FROM ASSIGNMENTS WHERE GROUP_ID = @group_id AND LESSON_ID = @assignmentId;",[GROUP_ID,LESSON_ID],async (err,rows)=>{
                var deleteStatusArr = [];
    ...
    const getAssignmentByGroupId = (groupId) => {
        return new Promise((resolve,reject) => {
             con.query('select * from ASSIGNMENTS WHERE GROUP_ID = @group_id group by @assignmentId',[groupId],(err,result)=>{
